...
function seealso(json) {
    var data = json.data[0];
    var html = '';
    for (var i = 0, len = data.seealso.length; i < len; i++) {
        var link = data.seealso[i];
        html += '<a href="' + link.url + '">' + link.bibl + '</a>\n';
    }
    var div = document.getElementById('seelinks');
    if (div) {
        div.innerHTML = html;
    }
    html = '<b>Mots clés</b> (<i>automatique</i>) : ';
    var first = true;
    var len = Math.min(15, data.nodes.length);
    for (var i = 0; i < len; i++) {
